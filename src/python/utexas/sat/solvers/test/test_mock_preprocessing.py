...
def test_mock_preprocessing_simple():
    ...
    def test_solver(preprocessor_name, solver_name, task_uuid, seconds, satisfiable, certificate):
        """
    ...
    yield (test_solver, "bar", "foo", task_uuids[0],  7.0, None,  None)
    yield (test_solver, "bar", "foo", task_uuids[0],  9.0, None,  None)
    yield (test_solver, "bar", "foo", task_uuids[0], 17.0, True,  None)
    yield (test_solver, "bar", "foo", task_uuids[1],  7.0, None,  None)
    yield (test_solver, "bar", "foo", task_uuids[1],  9.0, False, None)
    yield (test_solver, "bar", "foo", task_uuids[2],  7.0, None,  None)
    yield (test_solver, "bar", "foo", task_uuids[2],  9.0, True,  [42])

    # test the baz-fob combination on fake data
    yield (test_solver, "baz", "fob", task_uuids[0],  7.0, None,  None)
    yield (test_solver, "baz", "fob", task_uuids[0],  9.0, None,  None)
    yield (test_solver, "baz", "fob", task_uuids[0], 17.0, True,  None)
    yield (test_solver, "baz", "fob", task_uuids[1],  7.0, None,  None)
    yield (test_solver, "baz", "fob", task_uuids[1],  9.0, None,  None)
    yield (test_solver, "baz", "fob", task_uuids[1], 17.0, False, None)
    yield (test_solver, "baz", "fob", task_uuids[2], 17.0, None,  None)
